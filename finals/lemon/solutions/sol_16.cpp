...
static void build_masks() {
    ...
    for (int i = 0; i < 512; i++) {
        // Exclude mask size 8. Unlocks 503 valid states (>= 500).
        if (__builtin_popcount(i) != 8) g_valid_masks.push_back(i);
    }
}

static long long dp[10][10] = {0}; // dp[n][b]
static bool dp_built = false;

// Count valid strict binary trees with `n` internal nodes and remaining depth budget `b`
    ...
    for (int n = 2; n <= 8; n++) {
        for (int b = 1; b <= 8; b++) {
            ...
            for (int s1 = 0; s1 <= n - 1; s1++) {
                ...
                if (s1 == 0) left_ways = 1; // Left child is a leaf
                else for (int w = 1; w <= b; w++) left_ways += dp[s1][b - w];
                
                long long right_ways = 0;
                if (s2 == 0) right_ways = 1; // Right child is a leaf
...
string init(int subtask, int n, vector<int> p) {
    ...
    string B = "";
    // Tree Index (23 bits fits 7,303,164 configurations)
    for (int i = 22; i >= 0; i--) B += ((tree_index >> i) & 1LL) ? '1' : '0';
    // XOR Sum (9 bits)
...
int answer(int subtask, int n, string b, vector<int> uneaten) {
    ...
    for (int i = 0; i < 23; i++) tree_index = (tree_index << 1LL) | (long long)(b[i] - '0');
    
    int expected_xor = 0;
    for (int i = 0; i < 9; i++) expected_xor = (expected_xor << 1) | (b[23 + i] - '0');
