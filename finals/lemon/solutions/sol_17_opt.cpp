...
static void build_masks() {
    ...
    for (int i = 0; i < 512; i++) {
        ...
        // Exclude 8 to protect the fallback signal "Lemon is in T U P" (Y=17)
        // Exclude 9 to strictly guarantee Y <= 17 during normal encoding
        if (c != 8 && c != 9) {
...
static void build_families(int n) {
    ...
    // Fill up to 9 valid coprime k's dynamically just in case
    int cur_k = 1;
    while (k_vals.size() < 9) {
        if (gcd(cur_k, n) == 1) {
            bool used = false;
...
string init(int subtask, int n, vector<int> p) {
    ...
    vector<int> T_vec;
    // p is 1-indexed (padded with 0 at the start)
    ...
    // 3. Variable-length encoding of the family ID (length <= 3)
    long long map_val = g_f + 1;
    string B = "";
...
bool receive_fruit(int id, bool is_lemon) {
    int x = id - 1; // map 1-indexed id down to 0-based for logic
...
int answer(int subtask, int n, string b, vector<int> uneaten) {
    ...
    // 1. Decode family ID from variable-length bitstring
    long long map_val = 1;
    for (char c : b) {
    ...
    // 2. Identify all missing fruits (0-indexed)
    set<int> eaten;
    for (int i = 0; i < n; i++) eaten.insert(i);
    for (int id : uneaten) eaten.erase(id - 1);
    ...
    // This state is impossible to accidentally hit in Case 2 due to exclusions of popcount 8 and 9.
    if (c2 == 8 && c1 == 1) {
        for (int i = 0; i < n / 2; i++) {
            if (eaten_count[i] == 1) {
                int u = pair_elements[f][i][0];
                int v = pair_elements[f][i][1];
                if (!eaten.count(u)) return u + 1; // shift back up to 1-index
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (rank == 0) return i + 1; // shift back up to 1-index
        rank--;
    }
