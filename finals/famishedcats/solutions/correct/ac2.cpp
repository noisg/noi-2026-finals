...
void solve(int tc){
    ...
    vi ff=f;
    int D=0,fuel=x;
    FOR(0,n/2)if(ff[i]<ff[n-i-1])swap(ff[i],ff[n-i-1]);
    FOR(0,n){
        fuel-=d[i];
        if(fuel<0)break;
        D++,fuel+=ff[i];
    ...
    vi psum(n+1);
    psum[0]=x;
    FOR(0,n)psum[i+1]=psum[i]+(i==0?0:f[i-1])-d[i];
    FOR(0,n/2){
        fuel-=d[i];
        while(fuel<0){ //front
            SS++,add+=pq.top().F,fuel+=pq.top().F;
            pq.pop();
        }
        fuel+=f[i];
        if(f[i]<f[n-i-1])pq.push({f[n-i-1]-f[i],i});
        while(psum[n-i]+add<0 and n-i-1<D){ //back
            SS++,add+=pq.top().F,fuel+=pq.top().F;
            pq.pop();
        }
        if(i+1==D)break;
